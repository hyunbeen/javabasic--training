...
public class CustomerView extends JPanel 
{
	...
	public void registCustomer(){
		...
		cus.setCustName(tfCustName.getText());
		cus.setCustEmail(tfCustEmail.getText());
		cus.setCustAddr(tfCustAddr.getText());
		cus.setCustTel1(tfCustTel.getText());
		cus.setCustTel2(tfCustTelAid.getText());
		try {
	...
	public void searchByTel(){
		...
		try {
			Customer cus = db.selectByTel(tel);
			tfCustName.setText(cus.getCustName());
			tfCustAddr.setText(cus.getCustAddr());
			tfCustEmail.setText(cus.getCustEmail());
			tfCustTel.setText(cus.getCustTel1());
			tfCustTelAid.setText(cus.getCustTel2());
		} catch (Exception e) {
	...
	public void searchByName() {
		// 1. 입력한 전화번호 얻어오기
				...
				try {
					ResultSet rs = db.selectByName(name);
					rs.last();
					if(rs.getRow()==1) {
						tfCustName.setText(rs.getString("c_name"));
						tfCustTel.setText(rs.getString("tel_id"));
						tfCustTelAid.setText(rs.getString("s_tel"));
						tfCustAddr.setText(rs.getString("address"));
						tfCustEmail.setText(rs.getString("email"));
						return;
					}
					else {
					rs.first();
					MyDialog my = new MyDialog(rs);
					my.setSize(500,500);
					my.setVisible(true);
					}
				} catch (Exception e) {
	...
	public void updateCustomer() throws Exception{
		...
		cus.setCustTel1(tfCustTel.getText());
		cus.setCustTel2(tfCustTelAid.getText());
		int result = db.updateCustomer(cus);
		JOptionPane.showMessageDialog(null, result +"개 수정되었습니다");
	}
...
class MyDialog extends JDialog{
	...
	public MyDialog(ResultSet r1) {
		try {
			JTextArea ta = new JTextArea("tel\tname\ts_tel\taddress\temail\n");
			getContentPane().add(ta);
			
			
			do{
				
				ta.append(r1.getString("tel_id") +"\t");
				ta.append(r1.getString("c_name")+"\t");
				ta.append(r1.getString("s_tel")+"\t");
				ta.append(r1.getString("address")+"\t");
				ta.append(r1.getString("email")+"\t");
				ta.append("\n");
