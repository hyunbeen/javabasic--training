...
//<- 1 
	...
	*/
	...
	public void run() {
		while(true/*socClient.isConnected()*/) {
			try {
				...
				case BadukServerProtocol.All_ERASE : canvaserase();;break;
				}
					} catch (Exception ex) {
	...
	public void actionPerformed(ActionEvent ev) {
		...
		if(evt == tfMessage) { //채팅메세지 텍스트 필드
			chatMessageSend();
			
		}else if(evt == btnGame) { // 게임하기 버튼
			startGame();
		}else if(evt == btnStop) { // 그만하기 버튼
			stop();
		}
	}
	...
	void stop(){
		...
		BadukClientProtocol obj = new BadukClientProtocol();
		obj.setData("");
		obj.setState(BadukClientProtocol.ERASE);
		sendInformation(obj);
			...
			addMouseListener(new MouseAdapter() {
				...
				public void mouseReleased(MouseEvent e) {
					...
					if(bGamming) {
						...
						//바둑알들을 지정한 벡터에서 해당하는 row와 col 비교
						for( int i =0; i< badukRock.size(); i++){
							Baduk	al = (Baduk)badukRock.get(i);
							x = al.getCol();
							y = al.getRow();
							...
							if(x==col+1 && y==row) {win1++;}
							if(x==col+2 && y==row) {win1++;}
							if(x==col-1 && y==row) {win1++;}
							if(x==col-2 && y==row) {win1++;}
							if(x==col && y==row+2) {win2++;}
							if(x==col && y==row+1) {win2++;}
							if(x==col && y==row-1) {win2++;}
							if(x==col && y==row-2) {win2++;}
							if(x==col+1 && y==row+1) {win3++;}
							if(x==col+2 && y==row+2) {win3++;}
							if(x==col-1 && y==row-1) {win3++;}
							if(x==col-2 && y==row-2) {win3++;}
							if(x==col+2 && y==row-1) {win4++;}
							if(x==col+1 && y==row-1) {win4++;}
							if(x==col-1 && y==row+1) {win4++;}
							if(x==col-2 && y==row+2) {win4++;}
